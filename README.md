...
## 怎么用：一个 Claude Code 插件

这是一个 **Claude Code 插件（Skill）**。装好后，你只需要用大白话对它说话——
「把这份询价单转成报价单」「客户确认了，出 PI/CI/PL」——AI 全程帮你跑完，不用记任何命令。

👉 安装和使用看这里：[跳转](#use-skill)
...
当前版本针对**紧固件行业**优化（产品名中英翻译、分组逻辑是紧固件专属的）。

但核心框架**不限于紧固件**。只要你的业务符合以下条件：

- 客户通过 Excel 询价
- 按件（PCS）、千件（MPCS）、公斤（KG）或吨（TON）计价
- 需要生成报价单 / PI / CI / PL

就可以改一下产品翻译表来适配。

**适合的行业**：紧固件、垫圈、五金配件、标准件、线材、管件、法兰、轴承、密封件等工业零部件出口。

**不太适合**：服装（尺码矩阵复杂）、电子元器件（BOM 嵌套）、大宗散货（期货定价）、跨境电商（SKU 上万）。
...
<p align="center">
  <b>遇到问题？</b><br>
  <a href="https://github.com/Dangooy/trade-pipeline/issues">提一个 Issue</a>（不需要技术背景，写下你看到什么、期望什么即可）
</p>
